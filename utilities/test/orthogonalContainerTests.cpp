...
template<enum_t SubscribeSig, enum_t PostSig,
         enum_t TimerSig, uint32_t TimerIntervalTicks, const char * MockName>
...
using ComponentOne = TestComponent<TEST1_PUBLISH_SIG, TEST1_POST_SIG,
                                   TEST1_TIMER_SIG, TICKS_PER_SECOND*2, MockNames::CompOne>;
using ComponentTwo = TestComponent<TEST2_PUBLISH_SIG, TEST2_POST_SIG,
                                   TEST2_TIMER_SIG, TICKS_PER_SECOND*3, MockNames::CompTwo>;
...
TEST_GROUP(OrthogonalContainerTests) {
    ...
    static QP::QActive* StartAStaticallyAllocatedContainer() {
        ...
        myContainer.start(1, staticStorage.data(), staticStorage.size(), nullptr, 0);
        mock().checkExpectations();
...
TEST(OrthogonalContainerTests, can_start_a_container_with_three_components) {
    mock().ignoreOtherCalls();
    auto underTest = new OrthogonalContainer<ComponentOne, ComponentOne, ComponentOne>();
    CHECK_EQUAL(3, underTest->NumberOfComponents);
    underTest->start(1, eventStorage.data(), eventStorage.size(), nullptr, 0);
    delete underTest;
}

TEST(OrthogonalContainerTests, start_a_container_with_two_components_will_start_both_components) {
    auto underTest = CreateAndStartWithTwoTestComponents();
}

TEST(OrthogonalContainerTests, a_static_container_can_be_started_and_the_timer_works) {
...
TEST(OrthogonalContainerTests, container_with_two_components_will_route_published_event_to_appropriate_components) {
    auto underTest = CreateAndStartWithTwoTestComponents();
...
TEST(OrthogonalContainerTests, container_with_two_components_allows_for_timer_in_each) {
    auto underTest = CreateAndStartWithTwoTestComponents();
...
TEST(OrthogonalContainerTests, container_with_two_components_allows_for_posts_for_components_and_expected_components_process_expected_signals) {
