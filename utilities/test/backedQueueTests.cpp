...
TEST(BackedQueueTests, can_create_vector_backed_qequeue) {
    auto underTest = std::unique_ptr<VectorBackedQEQueue>(
        new VectorBackedQEQueue(10));
}

TEST(BackedQueueTests, vector_backed_qequeue_reports_size_plus_one_same_as_qequeue) {
...
TEST(BackedQueueTests, can_create_array_backed_qequeue) {
    auto underTest = std::unique_ptr<ArrayBackedQEQueue<10>>(
        new ArrayBackedQEQueue<10>());
}

TEST(BackedQueueTests, array_backed_qequeue_reports_size_plus_one_same_as_qequeue) {
    constexpr uint16_t MaxStorageForEvents = 10;
...
TEST(BackedQueueTests, can_push_up_to_max_events) {
    ...
    using ArrayTestType = ArrayBackedQEQueue<MaxStorageForEvents>;
    auto underTest = std::unique_ptr<ArrayTestType>(
        new ArrayTestType());
