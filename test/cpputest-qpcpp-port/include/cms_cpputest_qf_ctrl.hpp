...
namespace qf_ctrl {
    ...
    enum class MemPoolTeardownOption {
        CHECK_FOR_LEAKS,
        IGNORE
    };
    ...
    /// \param maxPubSubSignalValue - the system under test's maximum pub/sub signal value.
    /// \param ticksPerSecond - the system under test's expected ticks per second.
    /// \param pubSubEventMemPoolConfigs : empty list for internal defaults. Otherwise provide the desired configs.
    ///                                    Provided configs must be ordered from smallest to largest per QP
    ///                                    requirements.
    /// \param memPoolOpt - should a Teardown check for leaks in the pub/sub event memory pools?
    ...
    void PublishAndProcess(enum_t sig, PublishedEventRecorder* recorder = nullptr);
    void PublishAndProcess(QP::QEvt const * e, PublishedEventRecorder* recorder = nullptr);
